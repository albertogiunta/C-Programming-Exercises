...
/*Si scriva una funzione in C, denominata cerca, che ricerchi la presenza di un elemento in un vettore di interi. La funzione riceve in ingresso tre parametri:
 ...
 1. se il valore elem è presente nel vettore, allora la funzione restituisce l’indice della posizione alla quale si trova tale valore;
 2. se il valore elem è presente più volte, si restituisca l’indice di tutte le occorrenze;
 3. se il valore elem non è presente nel vettore, si restituisca -1.
 Si modifichi poi la funzione cerca definendo la funzione cerca_vettore_ordinato che faccia le stesse cose descritte in precedenza ma sul vettore ordinato*/
...
void stampa(int *indice, int ris){
	...
	/*stampa vettore*/
    if (ris == -1) {
        printf("\nIl valore cercato non è stato trovato all'interno del vettore.");
    } else {
        int length = ris;
        char singPlur;
        if (length == 1) {
        ...
        printf("\n\nIl numero %d è stato trovato (%d volt%c) in posizione: ", cerca, length, singPlur);
        for (int i = 0; i < ris; i++) {
            printf("%3d ", *(indice + i));
        }
    }
